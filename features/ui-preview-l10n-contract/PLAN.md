...
- Global function variables are not widget callbacks: a callee that resolves to a top-level variable (`debugPrint`) or a static field (a static hook) is skipped by the callback prose check, using the resolved element of the callee or `.call` target. Instance fields, parameters, and locals still report (coordinator confirmed).
...
- `avoid_magic_literals` exempts literals inside resolved `@Preview` functions, methods, and constructors (widget-previews.md:79-91 sample data such as `id: 'preview-1'`). Look-alike `Preview` annotations still report. `Preview`/`MultiPreview` subclass bodies get no class-wide exemption: the skill never describes them, and the class branch had no reachable reported case, so it was dropped; a map-key read inside a `MultiPreview` class still reports.
...
Current baseline: 2,204 tests pass after the change (2,191 after the first pass; five `avoid_missing_controller` tests removed; 18 added).

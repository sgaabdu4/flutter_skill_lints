...
- Coordinator audit follow-up (owner decision): `nested_linear_lookup_by_id` is now ERROR severity, like `linear_id_lookup_in_hot_path`. Its loop-variable match accepts a bare id (`item.id == id` in `for (final id in ids)`), not only `loopVar.field`. This closes the false negative in `common-patterns/debounce-gate-batch.md:145`.
...
- [x] Audit: a loop over an id list in a notifier now reports under both lookup rules, and both rules are ERROR severity (registration test). An unrelated `idx` key is not matched.
...
Current baseline: `dart analyze` finds no issues. `dart test` passes 2,169 tests.
Execution: One commit per issue, each green: #60, then #91, then #87, then the audit follow-up for the nested rule.

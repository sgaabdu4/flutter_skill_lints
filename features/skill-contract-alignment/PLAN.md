...
Result: Passed
Evidence: `dart format --set-exit-if-changed lib test` reported no changes, `dart analyze --fatal-infos lib test` found no issues, and `dart test` passed 2,682 tests with 1 skip. `python3 .hooks/hard-eng.py check --base origin/main --plan-stage Draft` passed every gate (lockfile, vulnerabilities, format, types-lint, security, import-boundaries, tests at 79.31% line coverage, dead-code-duplicates, performance, secrets, actionlint, zizmor). Registered counts: 229 skill rules, 237 skill codes, 235 additional rules, 273 additional codes, 508 unique codes, 63 fixes and 1 assist. Skill acceptance against the 5.12.0 docs: all 286 Dart fences were probed in consumer apps (206 unchanged fences from earlier extraction, 80 changed fences re-extracted). No DO fence reports a flutter_skill_lints diagnostic apart from 23 probe artifacts (several classes pasted into one file, probe-chosen layer directories, fragment wrappers and unused API-shape locals). Every WRONG example that names a lint reports it as an error; the one compile error is the dart-patterns-records.md line the skill marks as a compile-time error.

...
The package provides one public analyzer plugin for the
...
| Computed auto-dispose providers whose watched dependencies all resolve to `keepAlive` sources, in any file, become keepAlive too | `riverpod_auto_dispose_keepalive_dependencies` |
...
- `dart test --reporter expanded`: 1748 passing tests, 1 gated integration test skipped by default.
- `RUN_FLUTTER_PLUGIN_SMOKE=1 dart test test/integration_plugin_smoke_test.dart --reporter expanded`: passed. The temporary Flutter app loaded `flutter_skill_lints` with `riverpod_lint ^3.1.9`, emitted `avoid_null_bang`, `avoid_ref_read_inside_build`, and `missing_provider_scope`, and did not emit `server.pluginError`.

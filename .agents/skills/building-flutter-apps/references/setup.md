...
Copy [analysis_options.yaml](analysis_options.yaml) to the project root. It wires `flutter_skill_lints: ^0.11.0` and `riverpod_lint: ^3.1.9` under top-level `plugins:`, keeps strict inference enabled, and enforces `no_dynamic_casts` plus `no_raw_types`.
